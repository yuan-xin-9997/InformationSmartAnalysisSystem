...
`per_item` 事件邮件正文会把分析结果中包含的图表**内嵌**到正文（MIME `multipart/related` + `Content-ID: <cid>`，邮件客户端直接显示），同时**保留**图表与文章原文件（`local_folder` 源，PDF/docx/txt/md/html）作为附件一并发送。`aggregate` 事件无内嵌图、无附件。附件/图表读取复用文件服务的路径校验防穿越；单文件超 10MB 或不存在时跳过并记日志，不中断推送。

## 推送历史预览

「任务分析」页任务卡片「推送历史」展示该任务历次推送记录。**成功发送**的记录额外展示主题、附件数，并提供「预览邮件」按钮：

- 点击「预览邮件」弹出 iframe 弹层，以浏览器可渲染的版本展示本次推送实际发送的邮件正文（图表以 `data:` URL 内嵌；`sandbox="allow-same-origin"` 沙箱隔离）。
- 多批合并推送时各批以「第 i 封 / 共 N 封」分隔，主题取首封。
- 无邮件内容（`no_new`、发送前 `failed`）的记录不显示「预览邮件」按钮。
