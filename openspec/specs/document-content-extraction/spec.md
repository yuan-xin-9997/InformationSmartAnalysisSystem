...
当文本层不可用且视觉兜底启用时，系统 SHALL 用 PyMuPDF `page.get_pixmap()` 把 PDF 页面渲染为图片（可配置 DPI，默认 150），逐页调用多模态 LLM 提取该页正文文本，按页序拼接为整篇 `content` 落库。渲染页数 MUST 受可配置上限（默认 10 页）约束，超限时截断并在日志记录已截断。视觉兜底用的 LLM 模型 MUST 可独立于分析模型配置（未单独配置时复用 `llm.model`）。兜底抽取产生的 `content` MUST 为纯文本，沿用既有"先抽取、后分析"流程，分析侧无感知。

#### Scenario: 渲染页面并视觉提取正文
- **WHEN** 文本层不可用且视觉兜底启用
- **THEN** 系统把各页渲染成图片逐页调用多模态 LLM 提取文本，按页序拼接后作为 `content`，`extraction_method` 记为 `vision_llm`

#### Scenario: 页数超上限时截断并记录
- **WHEN** 一个 30 页扫描件 PDF 且最大渲染页数配置为 10
- **THEN** 系统只对前 10 页做视觉抽取，并在日志记录已截断

#### Scenario: 视觉模型可独立配置
- **WHEN** 配置了独立的视觉兜底模型（`extraction.vision_model`）
- **THEN** 视觉兜底调用使用该模型，而非 `llm.model` 分析模型

### Requirement: 正文抽取方式记录与追溯

`InfoItem` SHALL 新增 `extraction_method` 字段（取值 `text_layer` / `vision_llm` / `none`），记录该条目正文的抽取来源。文本层可用时记 `text_layer`；视觉兜底成功产出非空文本时记 `vision_llm`；两者均未产出有效文本时记 `none`。该字段 MUST 在分析结果接口只读返回，便于在前端与邮件中追溯正文来源。
...
#### Scenario: 记录视觉来源
- **WHEN** 视觉兜底成功产出非空正文文本
- **THEN** 该 `InfoItem.extraction_method` 记为 `vision_llm`
...
系统 SHALL 通过配置（`config/app.json` 的 `extraction` 节，支持 `ISAS_EXTRACTION_*` 环境变量覆盖）控制视觉兜底：启用开关（默认启用）、视觉模型、最大渲染页数、最小文本长度阈值、可读占比阈值、渲染 DPI。当视觉兜底未启用、LLM 未配置、或模型不支持视觉导致调用失败时，系统 MUST 优雅降级：记录警告日志，保留文本层原 `content`，不抛出异常、不中断同步或分析。

#### Scenario: 关闭兜底时仅用文本层
- **WHEN** `extraction.vision_fallback` 配置为 false
- **THEN** 文本层不可用时直接记 `extraction_method=none`，不调用视觉 LLM

#### Scenario: LLM 未配置时优雅降级
- **WHEN** 视觉兜底启用但 LLM `api_key` 未配置
- **THEN** 系统记录警告并跳过视觉抽取，`extraction_method=none`，同步继续不报错

#### Scenario: 视觉调用失败时降级
- **WHEN** 多模态 LLM 调用返回错误或超时
