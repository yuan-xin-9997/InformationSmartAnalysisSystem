"""Push endpoints: global SMTP config (read/save/test).

Consolidated (consolidate-task-analysis-page): the standalone push-rule
endpoints were removed -- push config is now 1:1 per task and managed through
``/api/analysis-tasks/{id}``. Only the global SMTP config remains here, surfaced
as the「邮件通道」section on the「任务分析」page; it requires the
``analysis_tasks`` page permission.
"""
